...
func test_public_surface_is_vendor_agnostic_and_slot_focused() -> void:
	...
	assert_eq(LoaderScript.VERSION, "0.3.0", "Wrapper version should reflect the landed fit-mode public image-loader slice")
	assert_true(bool(_loader.get_capabilities().get("supports_slots", false)), "Capabilities should advertise slot support")
	assert_true(bool(_loader.get_capabilities().get("supports_fit_mode", false)), "Capabilities should advertise fit-mode support")
	assert_eq(Array(_loader.get_capabilities().get("fit_modes", [])).duplicate(), ["stretch", "contain", "cover"], "Capabilities should advertise the shared fit-mode contract")
...
func test_loader_supports_res_user_absolute_and_remote_paths_through_the_public_wrapper() -> void:
	var res_result: Dictionary = _loader.load_image({"path": SAMPLE_RES_PATH, "slot": "background", "fit_mode": "cover"})
	...
	assert_eq(str(res_result.get("detail", {}).get("fit_mode", "")), "cover", "Result should preserve fit-mode intent")
	...
	var user_result: Dictionary = _loader.load_path(_user_sample_path, "card", "stretch")
	...
	assert_eq(str(user_result.get("detail", {}).get("fit_mode", "")), "stretch", "load_path should preserve stretch intent")

	var absolute_result: Dictionary = _loader.load_image({"path": _external_sample_path, "slot": "preview", "fit_mode": "contain"})
	...
	assert_eq(str(absolute_result.get("detail", {}).get("fit_mode", "")), "contain", "Absolute sample should preserve contain intent")

	var remote_result: Dictionary = _loader.load_image({"path": _remote_sample_url, "slot": "background", "fit_mode": "cover"})
...
func test_loader_places_textures_into_named_slots_and_maps_stretch_contain_and_cover() -> void:
	var background_surface := TextureRect.new()
	var card_surface := TextureRect.new()
	var preview_surface := TextureRect.new()
	add_child_autofree(background_surface)
	add_child_autofree(card_surface)
	add_child_autofree(preview_surface)

	var attach_background: Dictionary = _loader.attach_slot_surface("background", background_surface, "cover")
	var attach_card: Dictionary = _loader.attach_slot_surface("card", card_surface, "stretch")
	var attach_preview: Dictionary = _loader.attach_slot_surface("preview", preview_surface, "contain")
	...
	assert_true(bool(attach_preview.get("success", false)), "Preview slot surface should attach")
	assert_eq(str(_loader.get_slot_descriptor("card").get("fit_mode", "")), "stretch", "Slot descriptor should record stretch mode")
	assert_eq(str(_loader.get_slot_descriptor("preview").get("fit_mode", "")), "contain", "Slot descriptor should record contain mode")

	var background_result: Dictionary = _loader.load_image({"path": SAMPLE_RES_PATH, "slot": "background", "fit_mode": "cover"})
	assert_true(bool(background_result.get("success", false)), "Background slot should load packaged PNG")
	assert_eq(background_surface.stretch_mode, TextureRect.STRETCH_KEEP_ASPECT_COVERED, "Cover mode should map to covered keep-aspect placement")
	...
	var card_result: Dictionary = _loader.load_image({"path": SAMPLE_RES_PATH, "slot": "card", "fit_mode": "stretch"})
	...
	var preview_result: Dictionary = _loader.load_image({"path": SAMPLE_RES_PATH, "slot": "preview", "fit_mode": "contain"})
	assert_true(bool(preview_result.get("success", false)), "Preview slot should load packaged PNG")
	assert_eq(preview_surface.stretch_mode, TextureRect.STRETCH_KEEP_ASPECT_CENTERED, "Contain mode should map to centered keep-aspect placement")
	assert_not_null(preview_surface.texture, "Preview slot should receive the loaded texture")

	var update_result: Dictionary = _loader.set_slot_fit_mode("card", "contain")
	assert_true(bool(update_result.get("success", false)), "Slot fit mode should be mutable after attachment")
	assert_eq(card_surface.stretch_mode, TextureRect.STRETCH_KEEP_ASPECT_CENTERED, "Updating a slot to contain should immediately refresh presentation")

func test_loader_keeps_a_narrow_maintain_aspect_ratio_compatibility_seam() -> void:
	var legacy_surface := TextureRect.new()
	add_child_autofree(legacy_surface)

	var attach_result: Dictionary = _loader.attach_slot_surface("legacy", legacy_surface, true)
	assert_true(bool(attach_result.get("success", false)), "Legacy bool attach should still work")
	assert_eq(str(_loader.get_slot_descriptor("legacy").get("fit_mode", "")), "cover", "Legacy true should normalize to cover")

	var legacy_load: Dictionary = _loader.load_image({"path": SAMPLE_RES_PATH, "slot": "legacy", "maintain_aspect_ratio": false})
	assert_true(bool(legacy_load.get("success", false)), "Legacy maintain_aspect_ratio load should still work")
	assert_eq(str(legacy_load.get("detail", {}).get("fit_mode", "")), "stretch", "Legacy false should normalize to stretch")
	assert_false(bool(legacy_load.get("detail", {}).get("maintain_aspect_ratio", true)), "Legacy alias should still be reported for downstream callers")

	var update_result: Dictionary = _loader.set_slot_maintain_aspect_ratio("legacy", true)
	assert_true(bool(update_result.get("success", false)), "Legacy mutator should still work as a temporary seam")
	assert_eq(str(_loader.get_slot_descriptor("legacy").get("fit_mode", "")), "cover", "Legacy mutator should normalize back to cover")
